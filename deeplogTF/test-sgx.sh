...
if [ "$ROLE" == "make" ]; then
    ...
    make_custom_env
    taskset -c 0-1 stdbuf -o0 gramine-sgx python -u tf2-example-fl-distribute.py --task_index=0 --job_name=ps $PS_HOSTS $WORKER_HOSTS 2>&1 | runtime_logfilter | tee -a ps0-gramine-python.log &
    if [ "$DEBUG" != "0" ]; then
        wait && kill -9 `pgrep -f gramine`
    fi
elif [ "$ROLE" == "worker0" ]; then
    make_custom_env
    taskset -c 8-9 stdbuf -o0 gramine-sgx python -u tf2-example-fl-distribute.py --task_index=0 --job_name=worker $PS_HOSTS $WORKER_HOSTS 2>&1 | runtime_logfilter | tee -a worker0-gramine-python.log &
    if [ "$DEBUG" != "0" ]; then
        wait && kill -9 `pgrep -f gramine`
    fi
elif [ "$ROLE" == "worker1" ]; then
    make_custom_env
    taskset -c 12-13 stdbuf -o0 gramine-sgx python -u tf2-example-fl-distribute.py --task_index=1 --job_name=worker $PS_HOSTS $WORKER_HOSTS 2>&1 | runtime_logfilter | tee -a worker1-gramine-python.log &
    if [ "$DEBUG" != "0" ]; then
        wait && kill -9 `pgrep -f gramine`
    fi
fi
